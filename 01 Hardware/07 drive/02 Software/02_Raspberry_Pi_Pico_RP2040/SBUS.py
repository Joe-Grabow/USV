...
Program history:
16.10.2024    V. 1.0    Initiale Version (Author: Joe Grabow):
                        - PIO-basiertes UART-Receiving
                        - Datenpuffer & Interrupt-Handler
                        - SBUS-Kanaldecoder (1-16)
                        - Failsafe-Flags
26.03.2026    V. 1.1    (Author: S. Franke, C. Händel):
                        - SBUS-Kanaldecoder berichtigt
                        - Hinzugefügt: __debug_get_sbus_channel__()
                        - Hinzugefügt: get_normed_value()
...
zu beginnen.
- Normierung der Werte des SBUS-Channels von 172-1811 auf -1 bis +1
- Debug-Funktion zur Visualisierung der Bitwerte einzelner Kanäle
"""
...
class SBUSDecoder:
    ...
    @staticmethod
    def __debug_get_sbus_channel__(frame, channel):
        if not (1 <= channel <= 16):
            return None
    ...
    def get_sbus_channel(frame, channel):
        """
        Extrahiert einen Kanal aus einem SBUS-Frame und gibt den entsprechenden Wert im Bereich 172-1811 zurück.
        ...
        value_b11 = ((((frame[b1+2]&maskH)<<(8+shift))) | ((frame[b1+1]&maskM)<<shift) | ((frame[b1]&maskL)>>(8-shift)))
        
        return min([max([value_b11,172]),1811]) # Min: 172; Max: 1811
    
    # Verschiebung der Geradengleichung der Fernbedienungswerte von 172 bis 1811 zu normierten Werten: y = mx + n für get_normed_value():
    __n__ = const(1.20988408) # -Verschiebung = (-)1983/1639 (negative Werte in Konstante nicht abbildbar)
    # Anstieg m nicht als Konstante definierbar, da sonst Digitfehler

    @staticmethod
    def get_normed_value(value):
        """
        Bildet aus den Werten von 172 bis 1811 normierte Werte von -1 bis +1
        # 172 - 991 : negative Werte
        # 992: 0
        # 993 - 1811 : positive Werte
        :param value: Wert aus dem SBUS-Kanal (172-1811)
        :return: Wert von -1 bis +1
        """
        
        if value == 992:
            return 0
        elif value == 172: # Sicherstellen, dass Wert exakt - 1 und kein Rechenfehler entsteht
            return -1
        elif value == 1811: # Sicherstellen, dass Wert exakt 1 und kein Rechenfehler entsteht
            return 1
        else:
            return value*2/1639 - __n__ # y= m * x + n
        ...
        while True:
            if uart_receiver.buffer.is_full():  # Test auf vollen Enpfangspuffer
                ...
                if sbus_frame:  # wenn ein Frame erkannt, Kanal x ausgeben
                    ...
                    print('Trust:', channel)
                    print('Trust normed:', SBUSDecoder.get_normed_value(channel))
                    channel = SBUSDecoder.get_sbus_channel(sbus_frame, 2)
                    print('Rudder:', channel)
                    print('Rudder normed:', SBUSDecoder.get_normed_value(channel))
                    channel = SBUSDecoder.get_sbus_channel(sbus_frame, 14)
                    print('RC-Control on/off:', channel)
                    print('RC-Control on/off normed:', SBUSDecoder.get_normed_value(channel))
